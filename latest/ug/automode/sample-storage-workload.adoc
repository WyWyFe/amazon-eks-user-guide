...
Note that the `provisioner` specification `ebs.csi.eks.amazonaws.com` is meant for use only with EKS Auto Mode-enabled clusters and will not work to provision storage capacity for clusters that do not use EKS Auto Mode. 
`ebs.csi.eks.aws.com` is the intended storage class provisioner for clusters not using EKS Auto Mode. See more examples [here](https://github.com/kubernetes-sigs/aws-ebs-csi-driver/tree/master/examples/kubernetes).
